...
try {
    ...
    $accion = $_GET['accion'] ?? '';
    $controlador = new ControladorCamiones();
    $datos = $_POST;
    ...
    if (!esAdminMunicipal()) {
        responder(['ok' => false, 'mensaje' => 'Solo el administrador municipal puede modificar camiones.'], 403);
    }
    ...
    responder(['ok' => false, 'mensaje' => 'No se pudo guardar. Verificá que la matrícula no esté repetida.'], 400);
} catch (Throwable $error) {
    responder(['ok' => false, 'mensaje' => 'Ocurrió un error en la API de camiones.'], 500);
}